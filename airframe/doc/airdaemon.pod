...
Use of two dashes (--) after all B<airdaemon> command-line switches allows
...
normally during shutdown, B<airdaemon> will terminate normally as well, so it
is usually sufficient just to kill the pid in this file at shutdown.

=item B<--airdaemon-pidfile>=I<PID_FILE>

Write the process identifier of the B<airdaemon> process to I<PID_FILE>.
This option exists to facilitate the termination of the forked
B<airdaemon> process by shutdown scripts.
...
These options are used to specify how log messages are routed.  B<airdaemon>
