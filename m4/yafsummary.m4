...
AC_DEFUN([YAF_AC_WRITE_SUMMARY],[
    ...
    * GLIB:                         $yf_libstr"
    ...
    if test "x${YAF_ENABLE_LOCALTIME}" = "x1"
    then
        YF_BUILD_CONF="
    ...
    * Deep Packet Inspection:       NO"
    ...
    then
       ...
       yf_msg_ldflags=`echo "$yfpkg_lpath" | sed 's/^ *//' | sed 's/ *$//'`
       YF_BUILD_CONF="$YF_BUILD_CONF
    * nDPI Support:                 YES ${yf_msg_ldflags}"
    else
       YF_BUILD_CONF="$YF_BUILD_CONF
    * nDPI Support:                 NO"
    fi
    ...
    * IE/Template Metadata Export:  YES"
      ...
      YF_BUILD_CONF="$YF_BUILD_CONF
    * IE/Template Metadata Export:  NO"
    fi
    ...
    * Multi-threading available:    NO (reconfigure with --without-pic)"
    ...
    yf_msg_cflags="${YAF_CPPFLAGS} ${AM_CPPFLAGS} ${CPPFLAGS} ${WARN_CFLAGS} ${DEBUG_CFLAGS} ${AM_CFLAGS} ${CFLAGS}"
