...
static const GOptionEntry md_core_option[] = {
    ...
    {"reverse", 'R', 0, G_OPTION_ARG_NONE, &reverse,
     "Flip source and destination values to calculate" WRAP
    ...
    GError **err)
{
    ...
    rc = fbSessionAddTemplate(esession, TRUE, 999, template, NULL, err);

    if (!rc) {
        return NULL;
    }

    rc = fbSessionAddTemplate(esession, FALSE, 999, template, NULL, err);
