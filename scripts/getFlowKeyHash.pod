...
B<--pcap-per-flow> option) and the milliseconds since epoch time (helpful
for use with B<yaf>'s pcap-meta-file).

B<getFlowKeyHash> reads IPFIX from the file provided to B<--in> or from
standard input when no file names are specified and B<--sip4> or B<--sip6> and
B<--dip4> or B<--dip6> are not available.
...
Either B<--in> or the remaining options are required for B<getFlowKeyHash>
