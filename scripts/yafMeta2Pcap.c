...
static const GOptionEntry  md_core_option[] = {
    {"pcap-meta-file", 'f', 0, G_OPTION_ARG_STRING, &meta_file,
     "Pcap meta file (pattern) created by YAF." WRAP
     "Required.", "file" },
    {"pcap", 'p', 0, G_OPTION_ARG_STRING, &pcap_glob,
     "Pcap file (pattern) to read if full path is not" WRAP
     "specified in pcap_meta_file.", "file"},
    {"caplist", 'c', 0, G_OPTION_ARG_STRING, &caplist,
     "Ordered List of pcap files [--caplist] given to" WRAP
     "yaf with full paths to pcaps.", "file" },
    {"metalist", 'm', 0, G_OPTION_ARG_STRING, &metalist,
     "Ordered List of meta files created by yaf with" WRAP
     "full paths to files.", "file"},
    ...
    {"time", 't', 0, G_OPTION_ARG_STRING, &flowstarttime,
     "Time in milliseconds. Suggested, but not"
     "required.", "ms" },
    {"etime", 'e', 0, G_OPTION_ARG_STRING, &flowendtime,
     "End time in milliseconds. Suggested, but not" WRAP
     "required.", "ms" },
    {"window", 'w', 0, G_OPTION_ARG_INT, &timewindow,
     "Number of milliseconds past the start time to" WRAP
     "search for flow key hash. Search time window," WRAP
     "not exact time.",
     "ms"},
    {"packets", 'n', 0, G_OPTION_ARG_INT, &num_packets,
     "Use to limit number of packets searched" WRAP
     "for/found [all]", "num"},
    {"yaf-program", 'y', 0, G_OPTION_ARG_STRING, &yaf_prog_path,
     "The location of the YAF program to use. When" WRAP
     "not specified, yafMeta2Pcap assumes there is a" WRAP
     "program yaf on your $PATH", "path"},
    ...
    GError       **err)
{
    ...
    rc = fbSessionAddTemplate(session, TRUE, 999, template, NULL, err);
    if (!rc) {
        return FALSE;
    }
    ...
    void)
{
    ...
    if (metalist) {
        ...
        while (fgets(line, MAX_LINE, fp) && meta_files_num < MAX_LINE) {
            ...
            if (len) {
                meta_array[meta_files_num] = g_strndup(line, len);
                meta_files_num++;
            }
        }
        fclose(fp);
        ...
        meta_array = (char **)calloc(MAX_LINE, sizeof(char *));
        for (i = 0; i < gbuf.gl_pathc && meta_files_num < MAX_LINE; i++) {
    ...
    if (caplist) {
        ...
        while (fgets(line, MAX_LINE, fp) && pcap_files_num < MAX_LINE) {
            size_t len = strlen(line);
            if (len > 0 && '\n' == line[len - 1]) {
                --len;
            }
            if (len) {
                pcap_array[pcap_files_num] = g_strndup(line, len);
                pcap_files_num++;
            }
        }
        fclose(fp);
        ...
        pcap_array = (char **)calloc(MAX_LINE, sizeof(char *));
        for (i = 0; i < gbuf.gl_pathc && pcap_files_num < MAX_LINE; i++) {
