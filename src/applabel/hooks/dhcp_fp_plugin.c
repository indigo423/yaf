...
#define YAF_DHCP_FLOW_NAME_REV  "yaf_dhcp_rev"
#define YAF_DHCP_FLOW_NAME      "yaf_dhcp"
#define YAF_DHCP_FLOW_DESC      NULL
#define YAF_DHCP_FLOW_HIER      0x00000015
#define YAF_DHCP_FLOW_HIER_REV  0x00000016

/* DHCP OP. Level 1 #23/24 */
#define YAF_DHCP_OP_TID         0xC208
#define YAF_DHCP_OP_NAME_REV    "yaf_dhcp_op_rev"
#define YAF_DHCP_OP_NAME        "yaf_dhcp_op"
#define YAF_DHCP_OP_DESC        NULL
#define YAF_DHCP_OP_HIER        0x00000017
#define YAF_DHCP_OP_HIER_REV    0x00000018
...
static fbInfoElementSpec_t   yaf_dhcp_options_spec[] = {
    {"dhcpOptionList",              FB_IE_VARLEN, 0 },
    {"dhcpVendorCode",              FB_IE_VARLEN, 0 },
    {"reverseDhcpOptionList",       FB_IE_VARLEN, DHCP_REV },
    {"reverseDhcpVendorCode",       FB_IE_VARLEN, DHCP_REV },
    FB_IESPEC_NULL
};
    ...
    void      *yfctx)
{
    ...
    flowContext = g_slice_new0(ypDHCPFlowCtx_t);
    ...
    fbSession_t  *session)
{
    ...
    if (options_global) {
        if (!dhcp_uniflow_gl) {
            ...
            fbTemplateInfoInit(mdInfo, YAF_DHCP_OP_NAME_REV, YAF_DHCP_OP_DESC,
                               0, FB_TMPL_MD_LEVEL_1);

            if (!fbSessionAddTemplate(
                    session, FALSE, YAF_DHCP_OP_TID | flags,
                    revDhcpOpTemplate, mdInfo, &err))
            {
        ...
        mdInfo = fbTemplateInfoAlloc();
        fbTemplateInfoInit(mdInfo, YAF_DHCP_OP_NAME, YAF_DHCP_OP_DESC,
                           0, FB_TMPL_MD_LEVEL_1);

        if (!fbSessionAddTemplate(session, FALSE, YAF_DHCP_OP_TID,
                                  dhcpOpTemplate, mdInfo, &err))
        {
        ...
        if (!dhcp_uniflow_gl) {
            ...
            mdInfo = fbTemplateInfoAlloc();
            fbTemplateInfoInit(mdInfo, YAF_DHCP_FLOW_NAME_REV,
                               YAF_DHCP_FLOW_DESC, 0, FB_TMPL_MD_LEVEL_1);

            if (!fbSessionAddTemplate(
                    session, FALSE, YAF_DHCP_FLOW_TID | flags,
                    revDhcpTemplate, mdInfo, &err))
            {
                g_warning("Error adding template %02x: %s",
        ...
        mdInfo = fbTemplateInfoAlloc();
        fbTemplateInfoInit(mdInfo, YAF_DHCP_FLOW_NAME, YAF_DHCP_FLOW_DESC,
                           0, FB_TMPL_MD_LEVEL_1);

        if (!fbSessionAddTemplate(session, FALSE, YAF_DHCP_FLOW_TID,
                                  dhcpTemplate, mdInfo, &err))
        {
    ...
    void       **yfctx)
{
    ...
    newctx = g_slice_new0(yfDHCPContext_t);
