    ...
    GError                        **err)
{
    ...
    const struct tcp_header *tcph = NULL;
    ...
    iph = (const struct ip_header *)pkt;
    ...
    opt_ptr = (const uint8_t *)iph + g_htons(iph->tot_len);
    ...
    if (((uint8_t*)(tcph+1) - pkt) > (long)pktLen) {
        fatal(YAF_ERROR_PACKET_PAYLOAD,
    ...
    the packet data */
    if ( (pkt - ((uint8_t*)tcph) + tcpOptLen) < (long)pktLen) {
        openQuirks |= QUIRK_DATA;
    }
#endif  /* LOG_QUIRKS */
    /** figure out where the payloadPtr should be */
    payloadPtr = ((const uint8_t *)tcph) + (tcph->doff << 2);
    payloadPtr = ((payloadPtr - pkt) < (long)pktLen)  ? payloadPtr : NULL;
