 ...
 */
static gboolean yfSearchArray(GArray *ports, uint16_t dport){
    uint8_t i = 0;
    do {
    ...
    yfTCPInfo_t     *tcpinfo)
{
    ...
    /* Set the most significant byte to identify VxLAN overlay type
    as per IANA IPFIX Information Element 351, layer2SegmentId */
    ...
    *caplen -= vxlanh_len;
    ...
    /* Decode L2 headers and update the l2info to reflect the encapsulated frame */
    yfL2Info_t l2info;
    uint16_t   type = 0;
    ...
    yfTCPInfo_t     *tcpinfo)
{
    ...
    /* Verify we still have a full Geneve header after considering variable options */
    if (*caplen < geneveh_len) {
        ++ctx->stats.fail_geneve;
        return NULL;
    }
    ...
    if (!ctx->vxlanports) {
        /* Setting the most significant byte to 0x03 to identify Geneve overlay type */
    ...
    switch (key->proto) {
      ...
      case YF_PROTO_UDP:
        ...
        if (ctx->geneveports && yfSearchArray(ctx->geneveports, key->dp)) {
            if (!(pkt = yfDecodeGeneve(ctx, caplen, pkt, key, iplen, fraginfo, tcpinfo))) {
                return NULL;
            }
        }
        if (ctx->vxlanports && yfSearchArray(ctx->vxlanports, key->dp)) {
            if (!(pkt = yfDecodeVxLAN(ctx, caplen, pkt, key, iplen, fraginfo, tcpinfo))) {
                return NULL;
            }
        }
        break;
